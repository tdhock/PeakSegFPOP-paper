...
$f^\geq(\mu) = \min_{x\geq\mu} f(x)$.
  ...
  \begin{figure}
    ...
    \caption{Directed acyclic graph (DAG) representing dynamic
      programming computations for changepoint model with up-down
      constraints between adjacent segment means. Nodes in the graph
      repesent cost functions, and edges represent inputs to the the
      MinOfTwo sub-routine (Algorithm~\ref{algo:GFPOP}). There is one
      column for each data point and one row for each state: the
...
\begin{algorithm}
  ...
  \begin{ALC@g}
    ...
    \STATE If $p_{\text{new}} < P^*$: $\underline L,\underline p\gets L_{\text{new}},p_{\text{new}}$
    \STATE Else: $\overline L,\overline p\gets L_{\text{new}},p_{\text{new}}$
% > prob.i <- 3
...
\label{line:for-dp-t}
\begin{ALC@g}
  \STATE $\overline M\gets \lambda + \text{MinLess}(t-1, \underline C_{t-1})$ //cost of change up
  ...
  \STATE $\underline M\gets \text{MinMore}(t-1, \overline C_{t-1})$ //cost of change down
