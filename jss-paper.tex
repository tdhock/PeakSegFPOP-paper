...
package \pkg{changepoint}), or in $O(SN\log N)$ time using dynamic
...
$O(N\log N)$ time, $O(N\log N)$ disk, $O(\log N)$ memory.
  ...
  \begin{figure}
    ...
    \caption{Directed acyclic graph (DAG) representing dynamic
      ...
      the peak state $s=1$ at data point $t$ is $\overline C_t$ (top
      row); the optimal cost of the background noise state $s=0$ is
      $\underline C_t$ (bottom row). There is only one edge going to
