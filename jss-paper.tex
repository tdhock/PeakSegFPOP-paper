...
\begin{figure}
  ...
  \begin{tikzpicture}[->,>=latex,shorten >=1pt,auto,node distance=2.5cm,
    thick,main node/.style={circle,draw}]
    \node[main node] (peak_t1) {$\overline C_{i-1}$};
    \node[main node] (bkg_t1) [below of=peak_t1] {$\underline C_{i-1}$};
    \node[main node] (peak_t) [right of=peak_t1] {$\overline C_{i}$};
    \node[main node] (bkg_t) [right of=bkg_t1] {$\underline C_{i}$};
    ...
    (peak_t1) edge [dotted] node {$\overline C_{i-1}$} (peak_t)
    (peak_t1) edge [black, bend right] node [right] {$\overline C_{i-1}^{\geq}$} (bkg_t)
    (bkg_t1) edge [dotted] node[midway, below] {$\underline C_{i-1}$} (bkg_t)
    (bkg_t1) edge [black, bend left] node[right] {$\underline C_{i-1}^{\leq}+\lambda$} (peak_t)
  ...
  \caption{Directed acyclic graph (DAG) representing dynamic
    ...
    the peak state $s=1$ at data point $i$ is $\overline C_i$ (top
    row); the optimal cost of the background noise state $s=0$ is
    $\underline C_i$ (bottom row). There is only one edge going to
    $\underline C_2$ and $\overline C_2$ because the model is
