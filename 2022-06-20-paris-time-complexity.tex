...
\item Previous unconstrained model: not always up-down changes.
\item \alert{Interpretable: $P=(S-1)/2$ peaks (segments 2, 4, ...).}
\item H {\it et al.}, {\it ICML} 2015: $O(Sn^2)$ time approximate
  algorithm. 
\item \textbf{This paper}: $O(Sn\log n)$ time optimal algorithm.
\end{itemize}
...
\begin{frame}
  \frametitle{Labeled data setting}

  \includegraphics[width=\textwidth]{figure-good-bad}

  \begin{itemize}
  \item   Benchmark data with 2752 labeled data sequences:
    {\small \url{https://rcdata.nau.edu/genomic-ml/chip-seq-chunk-db/}}
\item  Goal is to learn a model for predicting the log penalty,
  $f(x)=\log \lambda$ with good peak prediction accuracy according to
  labels.
\end{itemize}

$$ S^*(\lambda) = \argmin_s L_s + \lambda s. $$
\end{frame}

% \begin{frame}
%   \frametitle{Comparing minimum label errors}
  
%   \includegraphics[width=\textwidth]{figure-PDPA-infeasible-error-compare}

%   \begin{itemize}
%   \item Proposed GPDPA with remove
%   \end{itemize}
...
\section{Linear Time Dynamic Programming for
  Computing Breakpoints in the Regularization Path of Models Selected From a Finite Set}
...
\begin{frame}
  ...
  From a Finite Set. Journal of Computational and Graphical Statistics
  (2021).

  R function \texttt{penaltyLearning::modelSelection}.
\end{frame}

\begin{frame}
  \frametitle{Three iterations of proposed algorithm}

  \includegraphics[width=\textwidth]{figure-three-iterations}

  \begin{itemize}
  \item For learning we would like an exact representation of the
    model selection function,
    $$ k^*(\lambda) = \argmin_k \underbrace{L_k + \lambda k}_{f_k(\lambda)}$$
  \item Each $f_k$ is a linear function; we propose a linear time
    algorithm which efficiently computes the intersection points
    (previous algorithms were quadratic time).
  \end{itemize}
  
\end{frame}

\begin{frame}
  \frametitle{Empirical iterations and theoretical bounds}

  \includegraphics[width=\textwidth]{figure-loss-small-evals}

  \begin{itemize}
  \item We proved linear upper and lower bounds on the total number of
    iterations $W_N$.
  \item We showed that in real data the number of iterations is closer
    to the lower bound than the upper bound.
  \end{itemize}
  
\end{frame}

\begin{frame}
  \frametitle{Empirical timings (synthetic data)}

  \includegraphics[width=0.7\textwidth]{figure-fullpath-db-binseg}

  \begin{itemize}
  \item Step1: binary segmentation.
  \item Step2: compute breakpoints in model selection function.
  \item Proposed linear algorithm faster than previous quadratic algorithm.
  \end{itemize}
\end{frame} 

\begin{frame}
  \frametitle{Empirical timings (real genomic data)}

  \includegraphics[width=0.7\textwidth]{figure-fullpath-grid-timing}
 
  Faster than approximate grid search algorithm and exact quadratic
  algorithm.
  
\end{frame}

\begin{frame}
  \frametitle{Accuracy in cross-validation experiments}

  \includegraphics[width=\textwidth]{figure-chipseq-cv}

  \begin{itemize}
  \item Learning using exact linear algorithm is just as good (and
    faster than) the approximate search algorithm using a large number
    of grid points.
  \end{itemize}
...
\begin{frame}
  \frametitle{Example run of sequential search algorithm using GFPOP}
...
\begin{frame}[fragile]
  ...
  \item Previous GPDPA was $O(N\sqrt N\log N)$ -- much too complex to
    ...
    data.
  \item Proposed GFPOP with sequential search is $O(N(\log N)^2)$ time,
    $O(\log N)$ memory, $O(N\log N)$ disk.
  \item Optimal models with large numbers of changepoints are now
...
\begin{frame}
  \frametitle{Not always faster}

  \includegraphics[width=\textwidth]{figure-binseg-quadratic-rigaill}
