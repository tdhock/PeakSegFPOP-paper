...
files <- Sys.glob("../chip-seq-paper/chunks/H*/*/peaks/macs.trained.RData")
...
for(file.i in seq_along(files)){
  ...
  for(param.name in names(peaks)){
    ...
    for(sample.id in names(regions.by.sample)){
      ...
      macs.peaks.error.list[[paste(chunk.name, sample.id, param.name)]] <- 
        data.table(chunk.name, sample.id, param.name, err.df)
    }
  }
}
macs.peaks.error <- do.call(rbind, macs.peaks.error.list)
save(macs.peaks.error, file="macs.peaks.error.RData")
