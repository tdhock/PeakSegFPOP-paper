...
for(model.file.i in i.vec){
  ...
  unsupervised.inf[[chunk.name]] <- do.call(rbind, segments.list)
  oracle.inf[[chunk.name]] <- do.call(rbind, oseg.list)
}
test <- do.call(rbind, unsupervised.inf)
stopifnot(!is.null(test))

save(unsupervised.inf, oracle.inf, 
     file="unsupervised.inf.RData")
