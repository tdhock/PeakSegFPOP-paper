...
Inferring the maximum likelihood changepoints and segments means
amounts to solving
\begin{equation}
...
Formally, the up-down constrained changepoint problem is defined as
\begin{align}
...
It is worth noting the connection between the up-down constrained
...
TODO clarify notation of functional/scalar cost and
constrained/unconstrained cost.
...
terms. The left term $\mathcal L_{K-1,t_{K-1}}$ is the previously
...
$\mathcal L_{K,K},\dots,\mathcal L_{K,n}$ is therefore $O(n^2)$ using
...
\begin{equation}
  u^*_{t_{K-1},n}=\argmin_{u_K\in\RR} h_{t_{K-1},n}(u_K),
\end{equation}
...
$u^*_{t_{K-1},n}$ that jumps in the correct direction (up or
...
\tau\in\{K-1, \dots, n-1\}\mid \mathcal U_{K-1,\tau}< u^*_{\tau,n} &\text{ if $K$ is even}\\
\tau\in\{K-1, \dots, n-1\}\mid \mathcal U_{K-1,\tau}> u^*_{\tau,n} &\text{ if $K$ is odd}\\
\end{cases}
\end{equation}
...
\widehat{\mathcal L}_{K,n} &=& \min_{\tau\in\mathcal I_{K,n}} 
\widehat{\mathcal L}_{K-1,\tau} + h^*_{\tau, n}
%\mathcal L_{K-1,\mathcal T_{K,n}} + h^*_{\mathcal T_{K,n}, n}\\
%\mathcal U_{K,n} &=& u^*_{\mathcal T_{K,n}, n}\\
\end{eqnarray}
Using this update rule, computing the entire matrix of constrained
cost values $\widehat{\mathcal L}_{k,t}$ for all $k\in\{1,\dots, K\}$
and $t\in\{1,\dots,n\}$ takes $O(Kn^2)$ time. This update
rule is a greedy heuristic; it does not always compute the optimal solution
to the constrained changepoint problem
(\ref{eq:segment-neighborhood-up-down}). We therefore propose a new
...
\begin{equation}
  \mathcal L_{K,n} = \min_{u_K} \underbrace{\min_{t_{K-1}} 
\mathcal L_{K-1,t_{K-1}}
+  h_{t_{K-1,n}(u_K)}}_{C_{K,n}(u_K)}.
\end{equation}
The idea, originally proposed by \citet{pruned-dp}, is to then recursively compute the $C_{K,n}$ functions via
\begin{eqnarray}
C_{K,n}(u_K) 
&=& \min_{\tau\in\{K-1,\dots,n-1\}} \mathcal L_{K-1,\tau} + h_{\tau,n}(u_K)
\\
&=& \ell(u_K, y_n) + \min\{\mathcal L_{K-1,n-1},\, 
\min_{\tau\in\{K-1,\dots,n-2\}} \mathcal L_{K-1,\tau} + h_{\tau,n-1}(u_K)
\}\label{eq:remove-last-data}\\
&=& \ell(u_K, y_n) + \min\{\mathcal L_{K-1,n-1}, C_{K,n-1}(u_K)\}
\label{eq:functional-recursion}
\end{eqnarray}
The equation (\ref{eq:remove-last-data}) is obtained by removing the
cost of the last data point $\ell(u_K, y_n)$ from the terms in the
min. The recursion (\ref{eq:functional-recursion}) states that the
functional cost $C_{K,n}(u_K)$ up to data point $n$ can be computed
using the functional cost $C_{K,n-1}(u_K)$ up to the previous data
point $n-1$. 
...
\begin{definition}[Constrained optimal cost with last segment mean $\mu$]
\label{def:fcc}
  Let $C_{K,n}(\mu)$ be the constrained optimal cost of the segmentation
...
To simplify the explanation of functional pruning for the constrained model, we first consider
...
\begin{theorem}[Dynamic programming for reduced isotonic regression]
\label{thm:gpdpa}
...
%   $C_{K,t+1}(u)$ we must have
%   $C_{K,t+1}(u) \leq C_{K,t}(u) + \ell(y_t,u)$ and also
%   $C_{K,t+1}(u) \leq C_{K-1,t}(u) + \ell(y_t,u)$ (TODO: should
%   there be a $C^\leq$ here?). Thus we have
%   $C_{K,t+1}(u) \leq \min \{ C_{K,t}(u) + C_{K-1,t}(u) \} +
%   \ell(y_{t+1},u)$.
...
% We consider the optimal segmentation $(\mathbf u, \mathbf t)\in\bar{\mathcal I}_{t+1}^K$ achieving the optimal $C_{K,t+1}(u)$.
...
% We can thus decompose $C_{K,t+1}(u)$ as
% $$C_{K,t+1}(u) = \sum_{k=1}^K
%   h_{t'_{k-1}, t'_k}(u_k) + \ell(y_{t+1},u).$$ 
% By assumption we would recover $\sum_{k=1}^K h_{t'_{k-1}, t'_k}(u_k) < C_{K,t}(u)$ 
% which is a contradiction
% by definition of $C_{K,t}(u)$. 
...
% We can then decompose $C_{K,t+1}(u)$ as
% $$C_{K,t+1}(u) = \sum_{k=1}^K
%   h_{t'_{k-1}, t'_k}(u'_k) + \ell(y_{t+1},u).$$ 
% By assumption we would recover $\sum_{k=1}^{K-1} h_{t'_{k-1}, t'_k}(u'_k) < C_{K-1,t}(u)$ which is a contradiction
% by definition of $C_{K-1,t}(u)$. 
% \end{description}
\end{proof}
...
% \label{def:fcc}
%   We refer to $C_{k,n}(u)$ as the optimal cost of the segmentation
%   with $k$ segments, up to data point $n$, with last segment mean
%   $u$. For the first segment $k=1$, we define
%   $C_{1,1}(u)=\ell(y_1,u)$ for the first data point, and
%   $C_{1,t}(u)=C_{1,t-1}(u)+\ell(y_t,u)$ for the other data
%   points $t>1$. For $k>1$ segments, we define
%   $C_{k,k}(u)=\ell(y_k, u)+C_{k-1,k-1}^\leq(u)$ for the $k$-th
...
% min-less cost functions $C_{k-1,t-1}^\leq:\RR\rightarrow\RR$ are
...
%   programming cost $C_{K,t}(u)$ is equivalent to the minimum cost
%   over all possible change points
%   $\min_{\tau\in[K-1,t)}C_{K-1,\tau}^\leq(u)+h_{\tau,t}(u)$.
% \end{lemma}
...
%   \begin{eqnarray}
%     C_{K,K+1}(u)
%     &=&\label{eq:proof_fcc1}\ell(y_{K+1},u)+\min\{C_{K-1,K}^\leq(u),\,C_{K,K}(u)\}\\
%     &=&\min\label{eq:proof_fcc2}
%         \begin{cases}
%           C_{K-1,K}^\leq(u)+\ell(y_{K+1},u)\\
%           C_{K-1,K-1}^\leq(u)+\ell(y_{K+1},u)+\ell(y_K,u)
%         \end{cases}\\
%     &=&\min\label{eq:proof_h}
%         \begin{cases}
%           C_{K-1,K}^\leq(u)+h_{K,K+1}(u)\\
%           C_{K-1,K-1}^\leq(u)+h_{K-1,K+1}(u)
%         \end{cases}\\
%     \label{eq:proof_tau}
%     &=&\min_{\tau\in[K-1,K+1)} C_{K-1,\tau}^\leq(u)+h_{\tau,K+1}(u)
%   \end{eqnarray}
%   Equations (\ref{eq:proof_fcc1}-\ref{eq:proof_fcc2}) result by
%   expanding $C_{K,K+1}$ and $C_{K,K}$ using
...
%   for $t+1$ data points.
%   \begin{eqnarray}
%     C_{K,t+1}(u)\label{eq:proof_fcct1}
%     &=&\ell(y_{t+1},u)+\min\{C_{K-1,t}^\leq(u),\,C_{K,t}(u)\}\\
%     &=&\min\label{eq:proof_induction_h}
%         \begin{cases}
%           C_{K-1,t}^\leq(u)+h_{t,t+1}(u)\\
%           \min_{\tau\in[K-1,t)} C^\leq_{K-1,\tau}(u)+h_{\tau,t+1}(u)
%         \end{cases}\\
%     &=&\min_{\tau\in[K-1,t+1)} C^\leq_{K-1,\tau}(u)+h_{\tau,t+1}(u)
%         \label{eq:proof_tau2}
%   \end{eqnarray}
%   Equation (\ref{eq:proof_fcct1}) results by expanding $C_{K,t+1}$
...
%   the optimal dynamic programming cost $\min_u C_{K,n}(u)$ is
...
%        +\min_{u_{K+1}\geq u_K}
%        h_{\tau, n} (u_{K+1})\\
% &=& \min_\tau\min_{u_K} C_{K,\tau}(u_K)\label{eq:proof_Ckt_induction}
...
% +C_{K,\tau}^\leq(u_{K+1})\\
% &=& \min_{u_{K+1}} C_{K+1,n}(u_{K+1}).\label{eq:proof_remove_tau}
% \end{eqnarray}
...
%   which exactly stores a cost function $C_{k,t}(u)$ for all values
...
% \label{line:min-max}
% \STATE $C_{1,1}\gets \text{OnePiece}(y_1, \underline y, \overline y)$
% \label{line:init-1}
% \STATE for data points $t$ from 2 to $n$:
% \begin{ALC@g}
%   \STATE $C_{1,t}\gets \text{OnePiece}(y_t, \underline y, \overline y) + C_{1,t-1}$
% \label{line:init-t}
% \end{ALC@g}
% \STATE for $k$ from 2 to $K$: for $t$ from $k$ to $n$: // DP
% \label{line:for-k-t}
% \begin{ALC@g}
%   \STATE $\text{min\_prev}\gets \text{MinLess}(t-1, C_{k-1,t-1})$ 
%   \label{line:MinLess}
%   % \STATE if $t=k$:
%   % \begin{ALC@g}
...
%   % \STATE else:
%   % \begin{ALC@g}
%   %   \STATE $\text{min\_new}\gets\text{MinOfTwo}(\text{min\_prev}, C_{k, t-1})$
%   % \end{ALC@g}
%     \STATE $\text{min\_new}\gets\text{min\_prev}$ if $t=k$, 
% else $\text{MinOfTwo}(\text{min\_prev}, C_{k, t-1})$
%   \label{line:MinOfTwo}
%   \STATE $C_{k,t}\gets \text{min\_new} + \text{OnePiece}(y_t, \underline y, \overline y)$
%   \label{line:AddNew}
% \end{ALC@g}
% \STATE for segments $k$ from 1 to $K$: // decoding
% \label{line:for-k-decoding}
% \begin{ALC@g}
%   \STATE $u^*,t',u'\gets \text{ArgMin}(C_{k,n})$
%   \label{line:ArgMin}
...
%     \STATE $t',u'\gets\text{FindMean}(u^*, C_{s,t'})$
%     \label{line:FindMean}
...
% $C_{k,t}$, which should be initialized as a $K\times n$ array of
% empty FunctionPieceList objects. The computation of $C_{1,t}$ for
...
% $C_{k-1,t-1}^\leq$). Line~\ref{line:MinOfTwo} sets the temporary
...
% stores the resulting FunctionPieceList in $C_{k,t}$.
...
% the ArgMin sub-routine to solve $u^* = \argmin_u C_{k,n}(u)$ (the
...
% (line~\ref{line:FindMean}). It takes the cost $C_{s,t'}$ of the
...
Each comparison shown in
Figure~\ref{fig:PDPA-infeasible-error-compare} results in a
