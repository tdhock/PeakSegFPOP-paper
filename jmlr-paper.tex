...
\begin{abstract}
  ...
  heuristics which are optimized for speed on these large data sets,
  but which suffer from sub-optimal peak detection accuracy.  In this
  paper we propose a new peak detection method for large data, based
  on a maximum likelihood segmentation model with
  ...
  between mean parameters in neighboring segments. We also propose a supervised learning algorithm for predicting the crucial penalty parameter, which determines the number of segments. We compare our
  algorithms to several others in an analysis of ChIP-seq data sets,
  for which computation time and peak detection accuracy are primary
  concerns. Our dynamic programming algorithm shows state-of-the-art peak detection
...
h_{t_{K-1},n}(u_K)\\
  ...
  \nonumber
\end{align}
We propose to simplify the equation above using the min-less operator:
...
$\mathbf{\hat y}^k\in\RR^{n_m}$ be the estimated mean vector with $k$
segments, and let $\ell(\mathbf y_m, \mathbf{\hat y}^k_m)$ be the
...
vector for the model with $k$ segments. The goal in~(\ref{eq:learn-f}) is to find the penalty function $f$ that
minimizes the total number of incorrect labels. This problem is non-convex, so we learn $f$ using
the L1-regularized linear model and convex relaxation previously
described \citep{HOCKING-penalties}. We used the implementation in R
package penaltyLearning, and $p=365$ features which depend on the data
size $n_m$, mean, standard deviation, quantiles, etc.
...
\subsection{GPDPA is more often feasible/optimal than other algorithms}
...
\subsection{GPDPA fits labels best in terms of minimum train error}
...
baselines). For each problem $m$ and peak model
$p\in\{1,\dots,P\}$, we computed the number of incorrect labels
$E[c^A_p(\mathbf y_m), L_m]$. For each problem $m$ we then compute
the minimum incorrect labels over all parameters,
...
We compare the distribution of min error differences $E_m^a-E_m^A$
...
complexity function. The second row of Supplementary Figure 4 plots
the distribution of test AUC values for one model complexity function
versus the other. For all three changepoint algorithms (CDPA, GPDPA,
PDPA), the linear penalty has a slightly larger test AUC ($p>0.05$ in
paired $t_{27}$-test). These data indicate that the linear model
complexity function should be preferred for accurate peak
detection in genomic data.

We also compare learning a penalty function with either one or
multiple parameters, given the linear model complexity function. The
first row of Supplementary Figure 4 plots the distribution of test AUC
values for one penalty function versus the other. For all three
changepoint algorithms, the multi-parameter penalty function has a
significantly larger test AUC ($p<0.07$). These data indicate that
learning a multi-parameter penalty function should be preferred for
accurate peak detection in genomic data.

\subsection{TODO test accuracy}
