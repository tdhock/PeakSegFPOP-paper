...
\end{eqnarray}
The main idea of the dynamic programming recursion
...
$t_{K-1}\in\{K-1, \dots, n-1\}$ can be computed in $O(n)$ time. Each
$h^*_{t_{K-1,n}}$ can be computed in $O(1)$ time; the vector of
...
and $t\in\{1,\dots,n\}$ takes $O(Kn^2)$ time. Furthermore, this update
rule does not compute the optimal solution to the constrained
changepoint problem (\ref{eq:segment-neighborhood-up-down}). We
therefore propose a new update rule in the next section which is both
faster and optimal.
...
respect to the last segment mean, we obtain the following equation for the optimal cost in $K$ segments up to data point $n$,
\begin{equation}
  \mathcal L_{K,n} = \min_{u_K} \underbrace{\min_{t_{K-1}} 
\mathcal L_{K-1,t_{K-1}}
+  h_{t_{K-1,n}(u_K)}}_{C_{K,n}(u_K)}.
\end{equation}
The idea is to then recursively compute the $C_{K,n}$ functions via
\begin{eqnarray}
C_{K,n}(u_K) 
&=& \min_{\tau\in\{K,\dots,n\}} \mathcal L_{K-1,\tau-1} + \sum_{i=\tau}^n \ell(u_K, y_i)\\
&=& \ell(u_K, y_i) + \min\{\mathcal L_{K-1,n-1},\, 
\min_{\tau\in\{K,\dots,n-1\}} \mathcal L_{K-1,\tau-1} + \sum_{i=\tau}^{n-1} \ell(u_K, y_i)
\}\\
&=& \ell(u_K, y_i) + \min\{\mathcal L_{K-1,n-1}, C_{K,n-1}(u_K)\}
\end{eqnarray}

\paragraph{Modification for constraints.} TODO
