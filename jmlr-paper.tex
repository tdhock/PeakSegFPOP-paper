...
computed a fixed feature vector $\mathbf x_m\in\RR^p$ for each problem
...
We consider two accuracy metrics based on the labels. First we 
...
genomic regions. It has been shown that these types of labels are
...
\item[HMCanBroad] is a another heuristic algorithm with unknown time
  ...
  $10^{-10}$ (few peaks) to $10^5$ (many peaks). We kept other parameters at default values.
\end{description}
...
labels $L_m$. We used the labels to compare the algorithms in terms
minimum train error, as follows.

For each problem $i$, and a given algorithm $A$, we computed a
sequence of $M$ peak models
$c^A_1(\mathbf y_m),\dots,c^A_M(\mathbf y_m)$ (varying number of
...
$E[c^A_m(\mathbf y_m), L_m]$. For each problem $i$ we then compute
the minimum number of incorrect labels over all parameters,
$E^A_m=\min_m E[c^A_m(\mathbf y_m), L_m]$.  
