...
\caption{\label{fig:evaluations-concave} Example of a $G(\lambda)$
  function which is maximized by
  Algorithm~\ref{algo:seq-search} in order to find the optimal
  model with $P^*$ peaks.}
\end{figure} 
  

After running GFPOP~(Algorithm~\ref{algo:GFPOP}) for a given data set
$\mathbf z$ and penalty $\lambda$, we obtain the solution
$(\mathbf m^\lambda,\mathbf s^\lambda, \mathbf c^\lambda)$ to
problem~(\ref{min:op-up-down}). The number of peaks is
\begin{equation}
  \label{eq:optimal_peaks}
  p(\mathbf c^\lambda) = \sum_{i=1}^{N-1}I(c^\lambda_i=1)
\end{equation}
and the total Poisson loss is
\begin{equation}
  \label{eq:total_loss}
  L(\mathbf m^\lambda, \mathbf z) = \sum_{i=1}^N \ell(m^\lambda_i, z_i).
\end{equation}
The 
...
\citet{crops} proposed a similar algorithm (CROPS) for computing
Changepoint models for a Range Of PenaltieS.
