...
\Plaintitle{Generalized Functional Pruning Optimal Partitioning for
...
package \pkg{changepoint}), or in $O(KN\log N)$ time using dynamic
...
Algorithm~\ref{algo:GFPOP} begins on line~\ref{line:initialize} by
...
The complexity of Algorithm~\ref{algo:GFPOP} is $O(N I)$, where $I$ is
...
We implemented the disk-based version of Algorithm~\ref{algo:GFPOP} in
...
space and time; for example in the data set above there are only 6921
lines used to represent 520000 data values.
...
(the four columns shown above, separated by tabs).
...
The main idea of the sequential search algorithm is to keep track of a
lower bound $\underline p<P^*$ and upper bound $\overline p>P^*$ on
the number of peaks computed thus far.  The algorithm starts with
$\lambda=0,\overline p=P_{\text{max}}$ and
$\lambda=\infty,\underline p=0$.  At each iteration of the algorithm,
we find the intersection of the affine functions
$g_{\underline p}(\lambda)=g_{\overline p}(\lambda)$, which leads to a
new candidate penalty
$\lambda_{\underline p,\overline p}=(L_{\overline p}-L_{\underline
  p})/(\underline p-\overline p)$.  As previously described
\citep{crops}, there are two possibilities for the solution to the
Optimal Partitioning problem:
\begin{itemize}
\item GFPOP($\lambda_{\underline p,\overline p}$) yields
  $\underline p$ or $\overline p$ peaks. In that case
  $\max_\lambda G(\lambda)=G(\lambda_{\underline p,\overline
    p})=g_{\underline p}(\lambda_{\underline p,\overline
    p})=g_{\overline p}(\lambda_{\underline p,\overline p})$ and there
  is no Optimal Partitioning model with $P^*$ peaks. We terminate the
  algorithm by returning the model with $\underline p$ peaks.
\item GFPOP($\lambda_{\underline p,\overline p}$) yields a new model
  with $\rho$ peaks. If $\rho=P^*$ then
  $\max_\lambda G(\lambda)=L_{P^*}$ and we return this
  model. Otherwise it must be true that
  $\underline p < \rho < \overline p$. If $\underline p<\rho < P^*$
  then we use $\rho$ for a new lower bound $\underline p$; otherwise
  we use it for a new upper bound $\overline p$.
  %% implying
  %% $G(\lambda_{\underline p,\overline p})=g_{\rho}(\lambda_{\underline
  %%   p,\overline p})$. In that case,
  %% $G(\lambda_{\underline p,\overline p})<g_{\underline
  %%   p}(\lambda_{\underline p,\overline p})$ and
  %% $G(\lambda_{\underline p,\overline p})<g_{\overline
  %%   p}(\lambda_{\underline p,\overline p})$. We cont
\end{itemize}
%% Because $G$ is defined as the
%% minimum of the $g_P$ (\ref{eq:G}), we know that
%% $G(\lambda_{\underline p,\overline p})\leq g_{\underline p}(\lambda)$
%% and
%% $G(\lambda_{\underline p,\overline p})\leq g_{\overline p}(\lambda)$.
...
Algorithm~\ref{algo:seq-search} summarizes the sequential search. It
uses GFPOP as a sub-routine to find the optimal model with at most
$P^*$ peaks. Its space complexity is the same as GFPOP: $O(N\log N)$
disk and $O(\log N)$ memory. Its time complexity is linear in the
...
The R code below computes the optimal model with 17 peaks:
...
some genomic data sets. We downloaded the \verb|chipseq| data set from
...
middle panel). It is clear that the green segment means in the optimal
5 peak model are a better fit, particularly for the peak on the
left. Furthermore the optimal 5 peak model actually has one equality
constraint, indicating that 5 peaks is likely to overfit these
data. Therefore we also computed the optimal 3 peak model (bottom
panel), which also has a higher log-likelihood than the 5 peak MACS2
model. Overall it is clear that our algorithms can be used to compute
models which are both more likely and more interpretable (with fewer
peaks) than heuristics such as MACS2.
