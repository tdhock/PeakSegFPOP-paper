...
The \pkg{PeakSegDisk} package contains several other helper functions
(which call the functions mentioned above). For example
\code{PeakSegFPOP\_vec} can handle integer vector data input
(including automatic conversion of repeated values to a more efficient
run-length encoding), and \code{PeakSegFPOP\_df} takes a data frame as
input:
