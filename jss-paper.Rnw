...
to compute the optimal model with 17 peaks. Note that iteration 1
evaluates both extreme penalties $\lambda\in\{0,\infty\}$ in parallel
(and $\lambda=\infty$ is the trivial model with 0 peaks that can be
