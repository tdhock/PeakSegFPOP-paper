...
segment-specific parameter for each segment $k$ by $m_k$. For the problem of detecting changes in ChIP-seq count data,
...
novelty of this paper with respect to that previous work is with
respect to:
\begin{description}
