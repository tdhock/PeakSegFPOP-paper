...
genome in specific samples and cell types. Each peak is separated from
...
background noise). An important sub-problem in analyzing these data is
identifying the precise genomic positions of the starts and ends of
these peak regions.
...
There has been substantial research into faster algorithms for solving the Optimal Partitioning problem. The Pruned Exact Linear Time (PELT)
algorithm of \citet{pelt} (\code{changepoint::cpt.mean} in \R) is $O(N)$ time if $K$ increases linearly with $N$. The Functional Pruning
Optimal Partitioning (FPOP) algorithm of \citet{Maidstone2016} (\verb|fpop::Fpop| in \R) is empirically $O(N\log N)$ time.
...
First, we present the first algorithm which directly solves the
Optimal Partitioning problem with up-down constraints between adjacent
segment means (GFPOP, Algorithm~\ref{algo:GFPOP}). The most similar
existing algorithm computes the solution to the corresponding Segment
Neighborhood problem in $O(KN\log N)$ time/space
...
our GFPOP implementation is $O(N\log N)$ time, $O(N\log N)$ disk, and
$O(\log N)$ memory, which makes it possible to compute optimal models
with many peaks for typical genomic data sets on common laptop
computers.
...
$O(\log K)$ calls to GFPOP. 
%Overall the proposed algorithm is thus $O( N \log(N)\log(K))$ time, $O(N\log N)$ disk, and $O(\log N)$ memory. 
In an analysis of benchmark genomic data sets, we show that
this algorithm can compute an optimal model with $\approx 1000$ peaks
for $N=10^7$ data using only $\approx 20$ calls to GFPOP. 
This translates into only hours of compute time and gigabytes of
storage, which is much less than weeks/terabytes which would be
required for the previous Segment Neighborhood solver. 

\paragraph{Novelty with respect to previous work.} This paper has
several substantial similarities and differences with previous work.
The main new algorithm presented in this paper is GFPOP, which
generalizes the FPOP algorithm of \citet{Maidstone2016}. FPOP was
originally proposed for use with Gaussian mean change-point problems,
but has also been used with robust loss functions, resulting in the
R-FPOP algorithm \citep{Fearnhead2019}. FPOP and R-FPOP perform the
same dynamic programming updates, but use different representations of
the cost function. In contrast, in this paper we propose a
generalization of FPOP for changepoint problems with inequality
constraints between adjacent segment means. The resulting algorithm,
GFPOP, is substantially novel because it uses different
dynamic programming update rules, which depend on the constraints. We
refer to the algorithm as ``Generalized'' because FPOP is the special
case corresponding to a constraint graph with only one node and edge
(Figure~\ref{fig:state-graph}, left).

Whereas an exact solver for the
up-down constrained changepoint model was previously proposed by
\citet{Hocking-constrained-changepoint-detection}, 
the current paper provides
a more efficient algorithm for computing the same model on the large genomic
data sets that are now common.
\begin{description}
...
sequential search algorithm for computing the optimal model with $P^*$
desired peaks (which is the input parameter). Both algorithms work by
repeatedly solving the Optimal Partitioning problem for different
penalties $\lambda$.
...
updates~(\ref{eq:dp-over}) and (\ref{eq:dp-under}) below.
...
which GFPOP uses $<1$GB of storage. The ATA disk we used was a TOSHIBA
MK1652GSX (attached to SCSI, cache/buffer size = 8192 KB). We observed
that disk storage is slower than memory storage by a constant factor
(1.7--2.3$\times$, Figure~\ref{fig:disk-memory-compare-speed}), which
was expected.
...
Pruning Optimal Partitioning (GFPOP) which computes the optimal
model for one penalty $\lambda$. We also proposed a sequential search
algorithm which repeatedly calls GFPOP with different penalties $\lambda$, in order to compute the most
likely model with at most $P^*$ desired peaks. 
...
of the number of data $N$. In the ChIP-seq data we examined, our GFPOP
implementation requires only $O(N\log N)$ time, $O(N \log N)$ space,
and $O(\log N)$ memory. We furthermore observed that using disk-based
storage is only a constant factor slower than memory-based
storage. Overall we showed that GFPOP can be used to compute optimal
peak models for up to $N=10^7$ data in reasonable amounts of time
(minutes).

Third, we studied the empirical complexity of the proposed sequential
search for $P^*$ desired peaks, as a function of the maximum number of
peaks $P_{\text{max}}=O(N)$ for a given data set. We observed that it
requires $O(\log(P^*) \log(P_{\text{max}}))$ calls to the $O(N\log N)$
GFPOP dynamic programming sub-routine. In the ChIP-seq data sets, for
computing models with $P^*>5$ desired peaks, we observed that the
sequential search uses fewer calls to the $O(N\log N)$ dynamic
programming sub-routine than the comparable Segment Neighborhood
algorithm. More generally, these results suggest that our proposed sequential search algorithm can be used to reduce computation times for other changepoint models, i.e.
Optimal Partitioning combined with the sequential search algorithm
should be preferred over the classical Segment Neighborhood approach.

Finally we analyzed the labels in our benchmark of genomic data,
which indicated that a desired number of peaks is $P^*=O(\sqrt
...
practical for the first time to compute optimal models with many desired peaks
$P^*$ in large genomic data sets.
