...
number of changepoints, although others exist \citep{mBIC,cleynen2013segmentation,capushe}.
...
well as modern modifications for genomic data sets \citep{mBIC}. There
...
with predicted peaks and segmentation model. Vertical lines dashed
lines are used to show changepoint variables with inequality
constraints ($c_i = 1,\, m_i < m_{i+1}$ or $c_i = -1,\, m_i > m_{i+1}$
in problem~\ref{min:op-up-down}) and equality constraints
($c_i \in\{-1,1\},\, m_i = m_{i+1}$). Since the \code{plot} method
returns a \code{ggplot} object, it can be further processed, e.g. to
zoom in on the part of the model with equality constraints:
