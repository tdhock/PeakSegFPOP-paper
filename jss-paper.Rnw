...
\citep{bellamn, segment-neighborhood}.  The classical algorithm for solving the
...
intervals, which results in a time complexity of $O(N^2)$.
However since the number of intervals in real data is empirically
$I=O(\log N)$ (see Figure~\ref{fig:intervals}), the overall time complexity
