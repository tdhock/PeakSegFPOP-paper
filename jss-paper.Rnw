...
algorithm of \citet{pelt} (\code{changepoint::cpt.mean} in \R) is
$O(N)$ time if $K$ increases linearly with $N$. The Functional Pruning
...
inference, and have linear run times if the number of changes
increases linearly in the number of observations \citep{stepR, FDRSeg}.
...
fit <- PeakSegDisk::PeakSegFPOP_df(Mono27ac$coverage, 1000.5)
class(fit)
gg <- plot(fit)
print(gg)
@
...
with predicted peaks and segmentation model. Vertical dashed green
lines are used to show changepoint variables with strict inequality
constraints, i.e. $c_i = 1,\, m_i < m_{i+1}$ or $c_i = -1,\, m_i >
...
5 peak model actually has one active equality constraint between adjacent
...
5 peak MACS2 model. The given example shows that our algorithm can be
...
used to create this manuscript (including all figures) is available at
