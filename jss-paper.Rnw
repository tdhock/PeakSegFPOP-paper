...
\citet{pruned-dp-new}, which empirically take
$O(K N \log N)$ time.  The novelty of this algorithm is a functional
...
Whereas a previous work proposed the first exact solver for the
up-down constrained changepoint model
\citep{Hocking-constrained-changepoint-detection}, this paper provides
a fast algorithm for computing the same model on the large genomic
