...
chromosome, and we would like to detect regions for which the count data are larger than the usual background level. These long contiguous genomic regions with large counts are referred to as ``peaks'' and they typically correspond to active genes/promoters in specific samples and cell types. Each peak is separated from other peaks by background/noise regions (i.e. it is impossible for two peaks to be directly adjacent to one another without intervening background noise). An important sub-problem in analyzing these data is identifying the precise genomic positions of the starts and ends of these peak regions. 
...
increase linearly in the number of observations \citep{stepR, FDRSeg}.
...
Note that in this constrained model, peaks can neither be predicted at the beginning nor at the end of the data. 
Although the CDPA
...
complexity of $O(N^2)$. One such example is when the data sequence
...
number of peaks required to achieved zero label errors in these data
...
package which uses the Gaussian loss/likelihood with a general
