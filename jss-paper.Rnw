...
$C_{s, i}$ cost functions. There are some pathological data sets
for which the algorithm computes $I=O(N)$
intervals, which results in a worst case complexity of
$O(N^2)$. One such example is when the data sequence always increases, which also results in the worst case of the original FPOP algorithm. However our empirical study suggests much faster average
complexity in real genomic data (Section~\ref{sec:timings}).
...
$P=1414$ peaks achieves zero label errors. Computing the
...
memory. In particular we found in the ChIP-seq data sets that it is always faster than Segment Neighborhood solvers for models with $P>5$ peaks. 
