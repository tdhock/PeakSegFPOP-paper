...
optimal cost function $C_{s,t}$. In the case of the Poisson loss,
each $C_{s,t}(\mu)$ is a piecewise function where each piece is of the
...
\STATE Initialize $2\times N$ empty PiecewiseFunction objects $C_{s,t}$ either in memory or on disk.
...
initializing the array $C_{s, t}$ of optimal cost functions (either in
...
(lines~\ref{line:op-MinMore}--\ref{line:op-MinOfTwo-under}). See
\citep{Hocking-constrained-changepoint-detection} for more details
about how the MinLess/MinMore/MinOfTwo sub-routines process the
PiecewiseFunction objects.
...
represent the $C_{s, t}$ cost functions. Theoretically there are some
...
data.dir <- file.path(tempdir(),"chr11:60000-580000")
dir.create(data.dir)
write.table(
...
\code{fit\$loss} component is one row that contains general
information about the computed model. We can see that the optimal
model for $\lambda=10^4$ had $P=7$ peaks ($K=15$ segments/$K-1=14$
changepoints). The \code{fit\$segments} component is used to
visualize three of those peaks in a subset of the data below.
