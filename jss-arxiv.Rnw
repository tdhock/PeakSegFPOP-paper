% -*- compile-command: "make jss-arxiv.pdf" -*-
...
\author{Toby Dylan Hocking, \code{Toby.Hocking@nau.edu} \\
    Guillem Rigaill, \code{Guillem.Rigaill@inra.fr}\\
    Paul Fearnhead, \code{p.fearnhead@lancaster.ac.uk}\\
    Guillaume Bourque, \code{guil.bourque@mcgill.ca}
 }
...
\begin{abstract}
We describe a new algorithm and \proglang{R} package for
...
the sequential search algorithm in the \pkg{PeakSegPipeline} package
