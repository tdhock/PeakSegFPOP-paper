...
\Abstract{We describe a new algorithm and \proglang{R}
  package for constrained optimal changepoint detection in large genomic data
  sets. We begin by reviewing existing dynamic programming algorithms
  for solving optimal changepoint detection problems, with and without
  constraints between adjacent segment means. In large genomic data,
  computing the constrained optimal model with many changepoints is
  too expensive using the previous $O(KN\log N)$ Segment Neighborhood
  algorithm (best model with $K$ segments for $N$ data). We propose
  new GFPOP algorithm which is much faster for models with many segments $K$, because it solves the
  constrained Optimal Partitioning problem in $O(N\log N)$ time (best
  model for a given penalty $\lambda$). We also propose an sequential
  search algorithm that computes the Segment Neighborhood solution by
  solving $O(\log K)$ Optimal Partitioning problems. We study the
  empirical time complexity of our algorithms in a benchmark of large
  genomic data sets, which have $O(\sqrt N)$ changepoints. Whereas the
  previous Segment Neighborhood algorithm would be $O(N\sqrt N\log N)$
  in this context, our proposed algorithm is only $O(N(\log N)^2)$,
  which is much faster. We show that our disk-based implementation in
  the \pkg{PeakSegPipeline} \R\ package can be used quickly compute
  constrained optimal models with many changepoints in genomic data
  sets of up to ten million observations.}
...
There are many applications, particularly within genomics, that involve detecting regions that deviate from a
usual/background behaviour, and where qualitatively this deviations lead to an increased mean of some measured 
signal (REFS). For example, with ChIPSeq data \citep{chip-seq} we have data of counts of ?? at different positions 
along a chromosome. Interest lies in detecting regions linked to protein binding, for which we would expect the 
counts to be higher than the usual background level.

One approach to detecting these regions is through algorithms that detect change in mean in data. This paper
builds on recent work of REF:HOCKING and presents a new changepoint algorithm, and its implementation within 
\R\ for doing so. This algorithm is based on modelling data as count data, and using the knowledge that we have
background regions and peak regions, with the latter having higher mean than the former. This imposes constraints
on the type of changes, with the mean of the data alternately increasing then decreasing in value. A particular
challenge with such methods is that for them to be widely used, the algorithms need to scale well in terms of 
both computational and memory costs to large data.

There are other approaches to tackling this type of problem, for example based on hidden Markov models (REFS). 
One drawback of such methods is that they assume the background mean does not change across large genomic regions, 
whereas such long-range changes are observed in many real data sets. See REF:HOCKING (AND EARLIER PAPER) for a 
detailed comparison of these methods with changepoint approaches. 
...
segment-specific parameter for the segment by $m_k$. For the problem of detecting changes in ChIP-seq data,
it is natural to model each data point as a realisation of a Poisson random variable, with the segment-specific 
parameter being the mean for that segment. Change detection is then an attempt to detect the points along the 
chromosome where the mean of the data changes. 

The algorithm we present is based on detecting changes via minimising a measure of fit to the data, 
with this measure of fit being the negative log-likelihood under our Poisson model. This is a specific 
implementation of a general approach to detecting changes, which introduces a loss for fitting a data-point $z$
with a parameter value $m$, which we denote $\ell(m,z)$. If we know the number of segments we can estimate the 
location of the segments via solving the following minisation problem
 \begin{align}
    \minimize_{\substack{
  \mathbf m\in\RR^{K}
\\
...
algorithm introduced by \citet{optimal-partitioning}. Optimal partitioning is an an 
...
algorithm of REF, which is implemented within \code{changepoint::cpt.mean}, and Functional Pruning
Optimal Partitioning (FPOP) of \citet{Maidstone2016} which is available in \R\ as
\verb|fpop::Fpop| \citep{fpop}. Both these algorithm have a computational cost that is linear in 
$N$ in situations where $K$ increases linearly with $N$. The FPOP algorithm has a computational cost 
...
the drawback is that you only get a single segmentation for a single value of the number of segments. 
...
concern is to find segmentations for a range of penalties, something that can be done efficiently (REF CROPS).
...
are no constraints between segment parameters. However, as deecribed above, constraints can be
useful when data need to be interpreted in terms of pre-defined
domain-specific states. In the ChIP-seq application the changepoint model needs to be interpreted in
...
Define our loss function to be the Poisson loss, $\ell(m,z)=m-z\log m$, and $\lambda>0$ to be a penalty for adding a changepoint. Then we can infer the number of segments and the location of the changes by solving
...
space and time while maintaining optimality \citep{Segmentor}; for
...
affine functions $g_P(\lambda)=L_P+\lambda(P-P^*)$. For an example $G$
...
panel). It is clear that the green segment means in the optimal 5 peak
model are a better fit, particularly for the peak on the
left. Furthermore the optimal 5 peak model actually has one equality
constraint between adjacent segment means, indicating that 5 peaks is
too many (overfitting) for these data. Therefore we also computed the optimal 3
peak model (bottom panel), which also has a higher log-likelihood than
the 5 peak MACS2 model. Overall it is clear that our algorithms can be
used to compute models which are both more likely and more
interpretable (with fewer peaks) than heuristics such as MACS2.
...
\subsection{Disk storage is slower then memory by a constant
  factor}

In this previous section, we discussed how tens of gigabytes of
...
model with $P$ peaks). The GPDPA requires exactly $2P$ iterations of
...
large number of peaks ($P>5$), it is clearly faster to use the
