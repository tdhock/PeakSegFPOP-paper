...
In this section we explain our new algorithm that can be used to
compute the solution to constrained optimization problems similar to
(\ref{PeakSegPDPA}).
...
% We need the
% following lemma, which states that the dynamic programming
...
as a test set, and train a model using all other folds. We  But Segmentor
has a low false positive and true positive rate, so it is easy to get
a low test error (just pick the model with the most peaks). But coseg
has a significantly higher AUC, about the same as PeakSegDP.
...
\begin{figure}[b!]
  \centering
  \includegraphics[width=\textwidth]{figure-min-train-error-some}
  \caption{Comparison of minimum train error for 2752 segmentation
    problems in the 7 ChIP-seq data sets. The new coseg algorithm is
    always at least as accurate as Segmentor, and mostly consistent with the
    heuristic PeakSegDP.}
