...
\nonumber\\
    \text{subject to} &\ \  \sum_{t=1}^{n-1} I(c_t \neq 0) = K, \\
& \ \ c_t = -1 \Leftrightarrow m_{t} > m_{t+1} \text{ (change down)}
\nonumber\\
& \ \ c_t = 0 \Leftrightarrow m_{t} = m_{t+1}  \text{ (no change)}
\nonumber\\
& \ \ c_t = 1 \Leftrightarrow m_{t} < m_{t+1} \text{ (change up)}
\nonumber
\end{align}
...
change-point, and the model complexity is the number of change-points
...
$P_t(\mathbf c)\in\ZZ$, which is problematic since in our biological
...
\nonumber\\
    \text{subject to} &\ \  \sum_{t=1}^{n-1} I(c_t \neq 0) = K, \\
& \ \ c_t = -1 \Rightarrow m_{t} \geq m_{t+1} \text{ (change down)}
\nonumber\\
& \ \ c_t = 0 \Rightarrow m_{t} = m_{t+1}  \text{ (no change)}
\nonumber\\
& \ \ c_t = 1 \Rightarrow m_{t} \leq m_{t+1} \text{ (change up)}
\nonumber\\
...
$(\mathbf m = \left[\begin{array}{cccc}1 & 2 & 2 & 3 \end{array}\right],
  \mathbf c = \left[\begin{array}{cccc}  1 & -1 & 1 \end{array}\right]$
is feasible for this problem (\ref{coseg}), 
but is not feasible for \ref{PeakSeg} (since $c_2=-1$ but $m_2 = m_3$).

Note that unlike the unconstrained model, in this model there are
sometimes larger models with larger cost. TODO example.
