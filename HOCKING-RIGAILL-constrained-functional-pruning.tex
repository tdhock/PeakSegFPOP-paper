...
optimal segmentation models (coseg, PeakSegDP, Segmentor), we define
an oracle penalty \citep{cleynen2013segmentation}, equivalent to the
oracle.1 model described by \citet{ We computed the area under the ROC curve (AUC) of each model
for each test set.  The previous algorithm with state-of-the-art
accuracy on this benchmark was PeakSegDP, and we observed that the new
coseg algorithm achieves comparable test AUC
(Figure~\ref{fig:test-error-dots}). In contrast, the unconstrained
Segmentor algorithm has TODO 
