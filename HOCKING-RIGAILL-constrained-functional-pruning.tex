...
\begin{lemma}[Equivalence of problem with reduced number of variables]
  \label{lemma:reduced}
...
\item $\text{MinLess}(f)$: an algorithm that inputs a
  ...
  (another FunctionPieceList). This algorithm should keep track See
...
\STATE TODO
\caption{\label{algo:minless}Optimal Partitioning Isotonic Regression (SNIR) solver.}
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
\begin{algorithmic}[1]
\STATE Input: FunctionPieceList $f_{\text{in}}$, a list of FunctionPiece objects.
