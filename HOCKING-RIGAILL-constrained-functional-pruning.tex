...
\begin{equation}
\FCC^{k+1}_{t+1}(\mu) = \min \{ \FCC^{k+1}_{t}(\mu)  , \underset{\mu' | a_{k,k}. \mu' + a_{k,k+1}. \mu  \geq  b_{k}}{\min} \{ \FCC^{k}_{t}(\mu') \}  \} + \gamma(y_{t+1}, \mu)
\end{equation}
...
such that the last mean $\mu_k=\mu'$ validates the $k-th$ constraint with $\mu_{k+1}=\mu$ and then add  $\gamma(y_{t+1}, \mu)$, that is:
 $$\underset{\mu' | a_{k,k}. \mu' + a_{k,k+1}. \mu  \geq  b_{k}}{\min} \{ \FCC^{k}_{t}(\mu') \} + \gamma(y_{t+1}, \mu).$$
\end{itemize}
