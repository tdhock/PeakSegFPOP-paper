...
data (0,4). The first step of the algorithm is computation of the
optimal cost in $k=1$ segment up to the data point $t=1$:
\begin{equation}
  \FCC_{1,1}(u) = (2-u)^2=4 - 4u + u^2.
\end{equation}
It is clear that this function can be stored exactly via the three
real-valued coefficients ($\text{constant}=4$, $\text{linear}=-4$,
$\text{quadratic}=1$). To compute the optimal cost in $K=2$ segments, we
first need to compute the min-less operator of this function:
\begin{equation}
...
segment endpoint $t'$ and mean $u'$. The optimal cost in $K=2$
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
After computing the optimal cost in $k=1$ segment up to data point $t=2$
\begin{equation}
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
we can compute the optimal cost in $K=2$ segments up to data point
$t=3$, which is defined as the minimum of two functions
$\FCC_{2,2}(u)$ and $\FCC_{1,2}^\leq(u)$ that have already been
computed. We therefore propose the following data structures and
...
\item $\text{OptimalMean}(p)$: an algorithm that takes one
  FunctionPiece object, and computes the optimal mean value. For the
  square loss FunctionPiece $p(u)=au^2+bu+c$ we have
...
\item FunctionPieceList: an ordered list of FunctionPiece objects,
  which exactly stores a cost function $\FCC_{k,t}$ and possible
  previous segment endpoints and means.
\item $\text{MinLess}(f)$: an algorithm that inputs a
  FunctionPieceList and outputs the corresponding min-less operator
  (another FunctionPieceList). 
\item
  $\text{MinOfTwo} (f_1, f_2)$: an algorithm that inputs two
...
\begin{algorithm}[H]
\begin{algorithmic}[1]
\STATE Input: FunPieceList $f_{\text{in}}$
\STATE Output: FunPieceList $f_{\text{out}}$
\STATE $\text{prev\_min\_cost} \gets\infty$
\STATE $\text{prev\_lower\_limit}\gets \text{LowerLimit}(f_{\text{in}}[0])$.
\STATE $i\gets 0$; 
\STATE While $i < $ length($f_{\text{in}}$):
\begin{ALC@g}
  \STATE FunPiece $p\gets f_{\text{in}}[i]$
  \STATE if prev\_min\_cost == $\infty$: // look for min in this interval.
  \begin{ALC@g}
    \STATE $u\gets \text{OptimalMean}(p)$ 
    \STATE if $\text{LowerLimit}(p)< u < \text{UpperLimit}(p)$:
    \begin{ALC@g}
      \STATE $\text{new\_interval}\gets [\text{prev\_lower\_limit}, u]$
      // Minimum found in this interval.
    \end{ALC@g}
    \STATE else:
    \begin{ALC@g}
      \STATE 
      $\text{new\_interval}\gets
 [\text{prev\_lower\_limit}, \text{UpperLimit}(p)]$
 // No minimum in this interval.
    \end{ALC@g}
  \end{ALC@g}
  \STATE else:
  \begin{ALC@g}
    \STATE if constant cost intersects this piece:
    \begin{ALC@g}
      \STATE 
      // add constant piece ending at intersection
    \end{ALC@g}
    \STATE else:
    \begin{ALC@g}
      \STATE 
      // add constant piece over entire interval
    \end{ALC@g}
  \end{ALC@g}
\end{ALC@g}
\caption{MinLess algorithm for computing min-less operator.}
\end{algorithmic}\label{algo:v2}
\end{algorithm}

The last step of the dynamic programming recursion is computation of
$\FCC_{K,n}(u)$. After that, we can begin computing optimal parameters
by starting with the last segment mean $u_K = \argmin_u \FCC_{K,n}(u)$.
...
We implemented the PeakSeg model
