...
#+BEGIN_SRC R
  ...
  change <- function(from, to, penalty, constraint){
  ...
  unconstrained <- list(
    state("anything"),
    change("anything", "anything", penalty, no.constraint))
  ...
  PeakSegFPOP <- function(data.vec, weight.vec, penalty){
    model <- list(
      loss("Poisson"),
      state("peak"),
      state("background"),
      start("background"),
      change("background", "peak", 0, non.decreasing),
      change("peak", "background", penalty, non.increasing),
      end("background"))
    GFPOP(model, data.vec, weight.vec)
  }
  ...
  PeakSegFPOP.start.or.end.up <- list(
    ...
    state("peak"),
    state("background"),
    change("background", "peak", penalty, non.decreasing),
    change("peak", "background", penalty, non.increasing))

  model.list <- PeakSegFPOP.start.or.end.up
  ...
  ## TODO functions for error checking, plotting, solving.

  reduced.isotonic.regression <- list(
    state("anything"),
    change("anything", "anything", penalty, non.decreasing))

  unimodal.regression <- list(
    state("can.change.up.or.down"),
    state("can.change.down"),
    change("can.change.up.or.down", "can.change.up.or.down", penalty, non.decreasing),
    change("can.change.up.or.down", "can.change.down", penalty, non.increasing),
    change("can.change.down", "can.change.down", penalty, non.increasing))

  unimodal.at.least.one.up <- c(unimodel.regression, list(
    state("start"),
    start("start"),
    change("start", "can.change.up.or.down", penalty, non.decreasing)))
