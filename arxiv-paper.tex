...
\caption{Examples of graphs for four models. Nodes represent states
  and edges represent changes. Constraint functions $g$ determine what
  types of changes are possible ($g_0$ any change, $g_\uparrow$
  non-decreasing, $g_\downarrow$ non-increasing). Note that the
  Unimodal regression state X should be interpreted as ``can change
  X'' e.g. up/down means ``can change up/down.'' }
  \label{fig:graphs}
\end{figure*}
...
following dynamic programming algorithm.  The initialization for the
first data point is $ C_{s,1}(u) = \ell(y_1, u)$ for all
states $s$. The dynamic programming update rule for all data points
$t>1$ is
\begin{equation}
...
point $t$ is
\begin{equation}
  M_{s,t}(u) = \min_{c\in E_s} C_{\underline v_c, t}(u) + \lambda_c,
\end{equation}
...
Note that for each constraint function $g$ there is a corresponding
sub-routine (e.g. $g_0$ MinUnconstrained, $g_\uparrow$ MinLess,
$g_\downarrow$ MinMore).
