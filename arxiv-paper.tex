...
distinct state. The edges $E=\{1,\dots,|E|\}$ is another set of
integers, each which represents one of the possible changes between
...
\caption{Examples of state graphs for four models. Nodes represent states
  and edges represent changes. Each change has a corresponding penalty
  $\lambda$, and a function $g$ that determines what types of changes
  are possible ($g_0$ any change, $g_\uparrow$ non-decreasing,
  $g_\downarrow$ non-increasing). Note that the Unimodal regression
  state X should be interpreted as ``can change X'' e.g. up/down means
  ``can change up/down.'' }
  \label{fig:graphs}
\end{figure*}
...
\begin{figure}[H]
 ...
 % \node[draw,circle] at (0, 6) (up_initial_t) {up t};
 % \node[draw,circle] at (0, 3) (up_down_t) {up/down t};
 % \node[draw,circle] at (0, 0) (down_t) {down t};

 % \node[draw,circle] at (4, 6) (up_initial_t1) {up t+1};
 % \node[draw,circle] at (4, 3) (up_down_t1) {up/down t+1};
 % \node[draw,circle] at (4, 0) (down_t1) {down t+1};
  ...
  \path[every node/.style={font=\sffamily\small}]
    (up_t) edge node {$C_{\text{up}, t}$} (up_t1)
    (up_down_t) edge node[midway, below] {$C_{\text{up/down}, t}$} (up_down_t1)
    (down_t) edge node {$C_{\text{down}, t}$} (down_t1)
    (up_t) edge node {$C_{\text{up}, t}^{g_\uparrow}+\lambda_{1}$} (up_down_t1)
    (up_down_t) edge [bend left] node {$C_{\text{up/down}, t}^{g_\uparrow}+\lambda_{2}$} (up_down_t1)
    (up_down_t) edge node [below left] {$C_{\text{up/down}, t}^{g_\downarrow}+\lambda_3$} (down_t1)
    (down_t) edge [bend right] node [midway, below] {$C_{\text{down}, t}^{g_\downarrow}+\lambda_4$} (down_t1)
;
% \draw [-] (up_initial_t) to[out=40,in=150] (up_initial_t1) {foo} ;
% \draw [->] (up_initial_t) edge (up_down_t1);

% \draw [->] (up_down_t) edge (up_down_t1);
% \draw [->] (up_down_t) edge (down_t1);
% \draw [-] (up_down_t) to[out=40,in=150] (up_down_t1);

% \draw [->] (down_t) edge (down_t1);
% \draw [-] (down_t) to[out=40,in=150] (down_t1);
 
\end{tikzpicture}
  \caption{Computation graphs.}
  \label{fig:comp-graphs}
\end{figure}

The pseudocode for this algorithm is stated below.
\begin{algorithm}[H]
\begin{algorithmic}[1]
