...
$E=\{1,\dots,|E|\}$ is another set of integers, each which represents
...
\textbf{Reduced Isotonic Regression}\\
  ...
  \path[every node/.style={font=\small}]
    (t) edge node {$C_{1, t}$} (t1)
    (t) edge [red, bend right] node [below] {$C_{1, t}^{g_\uparrow}+\lambda$} (t1)
;
\end{tikzpicture}
}
\parbox{2in}{
...
\textbf{Peak detection}\\
  ...
  \path[every node/.style={font=\small}]
    (peak_t) edge node {$C_{\text{peak}, t}$} (peak_t1)
    (peak_t) edge [red, bend right] node [right] {$C_{\text{peak}, t}^{g_\downarrow}+\lambda_\downarrow$} (bkg_t1)
    (bkg_t) edge node[midway, below] {$C_{\text{bkg}, t}$} (bkg_t1)
    (bkg_t) edge [red, bend left] node[right] {$C_{\text{bkg}, t}^{g_\uparrow}+\lambda_\uparrow$} (peak_t1)
;
\end{tikzpicture}
}
\parbox{2in}{
...
\textbf{Unimodal regression}\\
  ...
  \path[every node/.style={font=\small}]
    ...
    (up_t) edge [red, bend left] node [near end] {$C_{\text{up}, t}^{g_\uparrow}+\lambda_{1}$} (up_down_t1)
    (up_down_t) edge [red, bend left] node [above] {$C_{\text{up/down}, t}^{g_\uparrow}+\lambda_{2}\hspace{0.8cm}$} (up_down_t1)
    (up_down_t) edge [red] node [below left] {$C_{\text{up/down}, t}^{g_\downarrow}+\lambda_3$} (down_t1)
    (down_t) edge [red, bend right] node [midway, below] {$C_{\text{down}, t}^{g_\downarrow}+\lambda_4$} (down_t1)
;
\end{tikzpicture}
}
\caption{Computation graphs which represent the dynamic programming
  ...
  inputs to the $\min\{\}$ operation (\textcolor{red}{red for a
    changepoint}, black for no change).}
