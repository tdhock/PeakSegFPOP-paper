...
\begin{abstract}
  Peak detection is a central problem in genomic data, which are
  typically represented as a sequence of numbers along a chromosome. A
  ...
  achieve state-of-the-art peak detection accuracy in these
  ...
  solution in empirically log-linear time. In a benchmark of several
...
model of imposes an additional constraint \citep{HOCKING-PeakSeg}. Up
...
For a sequence of $n$ noisy data points, multiple changepoint
algorithms attempt to optimize $K$ real-valued segment mean parameters
and $K-1$ discrete changepoint parameters. This amounts to optimizing
segment means over a space that contains $O(n^{K-1})$ discrete
arrangements of changepoints. In general this problem can be solved in
$O(Kn^2)$ time using the original dynamic programming algorithm of
\citet{segment-neighborhood}. \citet{HOCKING-PeakSeg} used these ideas
to derive a Constrained Dynamic Programming Algorithm (CDPA) to infer
the PeakSeg model parameters. However, it has two issues. First, it is
a heuristic algorithm that is not guaranteed to recover the optimal
