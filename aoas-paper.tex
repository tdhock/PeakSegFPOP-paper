...
\begin{abstract}
  ...
  A recently proposed 
  constrained changepoint detection algorithm
  has been shown to increase peak detection accuracy,
  but its quadratic time complexity limits its application 
  to relatively small data sets. 
  In this paper we show how a functional pruning technique can be
  generalized to solve the constrained changepoint detection problem.
  This technique results in a new
  inference algorithm which computes the exact solution 
  to the constrained optimization problem in 
  log-linear time (on average in ChIP-seq data), achieves state-of-the-art 
  peak detection accuracy, and is orders of magnitude 
  faster than the previous quadratic time heuristic.
  Our implementation is available in the PeakSegOptimal
  package on CRAN.
\end{abstract}
