...
\begin{abstract}
  ...
  analysis of ChIP-seq data sets, for which computation time and
...
$K=5$ segments (because it discards the optimal changepoint after the . For these data, the optimal model [6, 6, 18, 15, 20,
...
enough predicted peaks) for a given labeled data vector $i$. Ideal
peak predictions would yield zero incorrect labels.
...
and then TPR, FPR, and AUC were computed for each fold ID.
...
\begin{equation}
  K_i^\lambda=\argmin_{k\in \mathcal K_i} \ell(\mathbf y_i, \mathbf{\hat y}^k_i) + \lambda k\left(
