...
\begin{abstract}
  ...
  from the bioinformatics literature, which are optimized for speed on
  ...
  non-strict inequality constraints between adjacent segment means.
  ...
  constrained problem.  In this paper we propose the first dynamic
...
means (and $K-1$ changepoints) using a Poisson model for $n$ non-negative count
data. In practice we compute a sequence of models for a range
...
this heuristic algorithm achieves
...
of the CDPA is $O(Kn^2)$. Since this is quadratic in the number of
data points $n$, it can be too slow for use on large genomic data sets
...
changes ($K$ distinct values). For real-valued data $y_i\in\RR$ we can
assume $y_t\stackrel{\text{iid}}{\sim}N(m_t,\sigma^2)$. Maximizing the
...
not have a large effect on changepoint model estimates. Data sizes are
...
peak predictions would yield zero incorrect labels (for example see Figure~\ref{fig:good-bad}).
