...
\begin{abstract}
  ...
  counts of DNA reads that are aligned to different locations of a chromosome into
  background and peak regions. These refer to regions with respectively lower and higher
  numbers of counts. A
  ...
  compute the optimal solution to constrained changepoint
  problems. This results in a new algorithm that computes the optimal
  solution and that empirically has a log-linear computational cost. In a benchmark of several
...
improving at a rapid pace, in progressively bigger genomic data
...
\begin{figure}[h]
  ...
  \caption{ChIP-seq read count data (grey lines) for one sample on a
    subset of chromosome~11. \textbf{Left:} the unconstrained maximum
    likelihood model with 5 segment means (horizontal green lines) has
    ...
    down). \textbf{Right:} the up-down constrained model is less
    likely, but each up change is followed by a down
...
background occur in long contiguous segments across the genome. Most
algorithms from the bioinformatics literature are heuristics with
several parameters that affect peak detection accuracy (window/bin
sizes, p-value thresholds, etc).
...
most likely model with exactly $K$ piecewise constant segment means.
However, it sometimes has several consecutive up changes, which makes
it non-trivial to interpret in terms of peaks
(Figure~\ref{fig:data-models}, left). To ensure that the segmentation
model is interpretable in terms of peaks and background, the PeakSeg
model of \citep{HOCKING-PeakSeg} imposes an additional constraint. Up
changes must be followed by down changes, and vice versa
(Figure~\ref{fig:data-models}, right). This constraint ensures that
...
best choice of segment means to minimise some loss function that measures how close the mean is to each 
...
possible segmentations to find the one that minimises this cost. In practice PeakSeg is used to find segmentations
for a range of $K$, and an appropriate criteria is then used to decide on which value of $K$ gives the best 
segmentation.

Despite its state-of-the-art accuracy, PeakSeg suffers from two issues. First, for any given
$K$, it is not guaranteed to find the optimal segmentation in terms of minimising our segment cost. Second,
the computational cost for PeakSeg is $O(Kn^2)$. The fact that this cost is quadratic in the number 
of data points means that it can be  too slow
for use on large genomic data sets. In this paper we propose a new algorithm
...
via minimising this cost. The problem of minimising the cost can be split into two cases, the first
...
that can solve the unconstrained minimisation, we are unaware of equivalent dynamic programming
algorithms for constrained version.  Our main contribution is described in 
Section~\ref{sec:algorithms}, where we generalize the functional pruning
technique of \citet{pruned-dp} so that it can be applied to the constrained minimisation problem. This
results in a new algorithm, which we call Generalized
Pruned Dynamic Progamming Algorithm (GPDPA), and which can cope not only with our up-down constraint, 
...
Let $\ell(y_t,m_t)$ be defined as minus log-likelihood for this marginal model.
We can measure how well our mean vector fits the data via the cost
$\sum_{t=1}^n \ell(y_t,m_t)$, and estimate the mean vector through minimising this cost.
...
A common approach to detecting changes in mean is to minimise such a loss 
...
\begin{align}
  ...
  \nonumber 
\end{align}
To simplify description of our approach to solving constrained minimisation problems, such as with our
...
Whilst there are many algorithms that solve the unconstrained minisation problem
(\ref{eq:optimal_segment_neighborhood}) \cite[]{segment-neighborhood,pruned-dp,phd-johnson} or related 
versions of it \cite[]{optimal-partitioning,pelt,fpop,flsa}, fewer algorithms exist for constrained
minimisation problems such as (\ref{eq:reduced}) \cite[though see][for an approach for reduced isotonic regression]{hardwick2014optimal}. Our contribution in this paper is proving that the functional pruning
technique of \cite{pruned-dp} and \cite{fpop} can be generalized to constrained changepoint models such as reduced
isotonic regression and the up-down constrained that appears in peak detection problems
(Table~\ref{tab:contribution}). Our resulting Generalized Pruned
Dynamic Programming Algorithm (GPDPA) enjoys $O(Kn\log n)$ time
complexity (on average in our empirical tests of real ChIP-seq data
sets), and works for any changepoint model with affine constraints
between adjacent segment means (including isotonic regression,
unimodal regression, and peak detection).
...
Section~\ref{sec:results_time}. Thus the average empirical time complexity of
...
Section~\ref{sec:results-chip-seq} is on ChIP-seq count data
$y\in\ZZ_+ = \{0, 1, 2, \dots\}$.
...
computes the $K$-segment model for a single penalty constant $\lambda$
