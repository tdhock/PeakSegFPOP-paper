...
animint2dir(viz, "figure-constrained-PDPA-normal-simple")
...
gg.pruning <- ggplot()+
  ...
  geom_line(aes(mean, cost),
            color="grey",
            size=2,
            data=envelope[timestep==ti & total.segments==n.seg,])+
  scale_color_manual("previous\nsegment\nend", values=break.colors)+
  geom_line(aes(mean, cost, group=paste(data.i.fac, piece.i),
                color=data.i.fac),
            data=cost.lines[timestep==ti & total.segments==n.seg,])+
  geom_point(aes(min.cost.mean, min.cost, color=data.i.fac),
             data=minima[timestep==ti & total.segments==n.seg,])
print(gg.pruning)
...
pdf("figure-constrained-PDPA-normal-simple.pdf")
print(intervalsPlot)
dev.off()
