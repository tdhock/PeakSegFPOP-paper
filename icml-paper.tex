...
(genomic regions with large values, representing protein binding or
modification) and the negative class is background noise (small
values).
...
that the quadratic $O(Kn^2)$ time complexity for $n$ data points makes
...
takes on average $O(Kn\log n)$ time whereas the DPA takes $O(Kn^2)$
time.
