...
the time complexity of the algorithms, while maintaining
optimality. \citet{pelt} proposed an inequality pruning technique,
which results in an algorithm of $O(n\log n)$ time
complexity. \citet{pruned-dp} and \citep{phd-johnson} independently
discovered a functional pruning technique, which also results in
algorithms of $O(n\log n)$ time complexity. \citet{fpop} provides a
clear discussion on the differences between the two pruning
techniques.
...
than searching all possible change-points, a constraint is used to
search a smaller, more interpretable model space. For example,
\citet{haiminen2008algorithms} propose an $O(Kn^2)$ algorithm for
unimodal regression, which enforces no up changes after the first down
change. \citet{HOCKING-PeakSeg} propose an $O(Kn^2)$ algorithm for
peak detection, which enforces a down change after each up change, and
vice versa.
...
\citep{reduced-monotonic-regression}. There are sub-quadratic time
algorithms for solving this problem \citep{hardwick2014optimal}. 
...
% MACS and HMCanBroad \citep{MACS, HMCan}. 
