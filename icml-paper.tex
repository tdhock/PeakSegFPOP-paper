...
\begin{abstract}
  ...
  to solve such constrained changepoint detection problems. This
  leads to a new algorithm with empirical time complexity that is
  log-linear in the amount of data. This algorithm achieves
  state-of-the-art accuracy in a benchmark of several genomic data
  sets, and is orders of magnitude faster than existing algorithms
  that have similar accuracy.
  % changepoint detection is a central problem in time series and
...
\end{equation}
It is clear that this function can be stored for all values of $\mu$
via the three real-valued coefficients ($\text{constant}=4$,
$\text{linear}=-4$, $\text{quadratic}=1$). To compute the optimal cost
in $K=2$ segments, we first compute the min-less operator (red curve
on left of Figure~\ref{fig:compare-unconstrained}),
\begin{equation}
  \FCC_{1,1}^\leq(\mu) =
...
\begin{align}
  ...
  &\text{subject to} && u_{k-1} \leq u_k\ \forall k\in\{2,4,\dots\},
  \nonumber\\
...
\begin{align}
  ...
  \sum_{k=1}^K h_{t_{k-1}, t_k}(u_k)\\
  &\text{subject to} && g_k(u_k, u_{k+1})\leq 0\ \forall k\in\{1,\dots,K-1\}.
  \nonumber
 \\
&&& 0=t_0<t_1<\cdots<t_{K-1}<t_K=n.
\nonumber
