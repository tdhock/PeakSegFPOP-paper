...
\begin{abstract}
  ...
  solve change-point detection problems with affine constraints on
...
$y_i \sim \mathcal N(m_i, \sigma^2)$ and performing maximum likelihood
inference results in a convex minimization problem with affine constraints,
...
indicator function $I$ (\ref{eq:Sk}). Nonetheless, the optimal
...
an explicit link to an equivalent problem with fewer optimization
variables.
...
% change-point problems. We begin by discussing how to solve a special
% case, the reduced isotonic regression problem
% (\ref{eq:reduced}).

We begin by discussing an algorithm for solving the reduced isotonic
regression problem (\ref{eq:reduced}), then explain how the algorithm
generalizes to other constrained change-point problems.
...
\begin{definition}[Optimization space with fewer variables]
\label{def:Ibar}
...
PDPA (right of Figure~\ref{fig:timings}). It is clear that the
$O(n^2)$ CDPA algorithm is slower than the other two algorithms,
especially for larger data sets. For the largest count data vector
...
about two minutes. HMCanBroad and MACS took TODO minutes on this data
set. The total computation time to process all 2752 count data vectors
was 156 hours for the CDPA, and only 6 hours for the GPDPA (26 times
...
for HMCanBroad).
% Note that since these are not change-point detection algorithms,
...
that constrained optimal change-point detection algorithms are more
...
constrained change-point models. This will be an interesting direction
