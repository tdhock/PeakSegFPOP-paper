...
for(model.file.i in i.vec){
  ...
  unsupervised.Segmentor[[chunk.name]] <- do.call(rbind, segments.list)
  oracle.Segmentor[[chunk.name]] <- do.call(rbind, oseg.list)
}
test <- do.call(rbind, unsupervised.Segmentor)
stopifnot(!is.null(test))

save(unsupervised.Segmentor, oracle.Segmentor, 
     file="unsupervised.Segmentor.RData")
