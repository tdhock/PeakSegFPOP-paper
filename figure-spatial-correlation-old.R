...
for(file.i in 1:nrow(files.dt)){
  ...
  if(!file.exists(bed.gz)){
    ...
    system("scp th798@monsoon.hpc.nau.edu:genomic-ml/guillimin-projects-thocking/blueprint/bed/McGill0002/H3K4me3.bed.gz .")
  ...
  rir <- some.reads[, .(
    count=.N
  ), by=list(chromStart, chromEnd)]
  reads.in.region.list[[file.i]] <- data.table(f, rir) 
...
reads.in.region <- do.call(rbind, reads.in.region.list)
cov.dt <- do.call(rbind, cov.dt.list)
peaks.dt <- do.call(rbind, peaks.dt.list)[status=="peak"]

ChIPreads <- reads.in.region[, .(
  experiment, chrom, chromStart, chromEnd, count)]
prompt(ChIPreads, "~/R/PeakSegDisk/man/ChIPreads.Rd")
save(ChIPreads, file="~/R/PeakSegDisk/data/ChIPreads.RData")
